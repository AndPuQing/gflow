...
fn archived_log_file_path(log_path: &std::path::Path, current_attempt: u32) -> PathBuf {
    ...
    if current_attempt > 0 {
        log_path.with_file_name(format!("{file_name}.attempt-{}", current_attempt - 1))
    } else {
...
fn prune_retry_log_history_in_dir(
    ...
    let retained_prefix = format!("{job_id}.log.attempt-{}", successful_attempt - 1);
    let managed_prefix = format!("{job_id}.log.attempt-");
    ...
    for entry in std::fs::read_dir(log_dir)? {
        ...
        if (file_name.starts_with(&managed_prefix) || file_name.starts_with(&legacy_prefix))
...
mod tests {
    ...
    #[test]
    fn archived_log_file_path_uses_attempt_number_for_retries() {
        let log_path = PathBuf::from("/tmp/42.log");
        let archived = archived_log_file_path(&log_path, 2);
        assert_eq!(archived, PathBuf::from("/tmp/42.log.attempt-1"));
    }
    ...
    fn uniquify_archive_path_adds_suffix_when_target_exists() {
        ...
        let archive_path = temp_dir.path().join("42.log.attempt-0");
        ...
        assert!(unique
            ...
            .unwrap()
            .to_string_lossy()
            .starts_with("42.log.attempt-0.dup."));
    }

    #[test]
    fn prune_retry_log_history_keeps_only_previous_attempt_on_success() {
        let temp_dir = tempfile::tempdir().unwrap();
        let log_dir = temp_dir.path();

        std::fs::write(log_dir.join("42.log.attempt-0"), "first failure").unwrap();
        std::fs::write(log_dir.join("42.log.attempt-1"), "second failure").unwrap();
        std::fs::write(log_dir.join("42.log.old.123"), "legacy").unwrap();
        std::fs::write(log_dir.join("99.log.attempt-0"), "other job").unwrap();

        prune_retry_log_history_in_dir(log_dir, 42, 2).unwrap();

        assert!(!log_dir.join("42.log.attempt-0").exists());
        assert!(log_dir.join("42.log.attempt-1").exists());
        assert!(!log_dir.join("42.log.old.123").exists());
        assert!(log_dir.join("99.log.attempt-0").exists());
    }
}
