...
pub async fn handle_up(gpus: Option<String>) -> Result<()> {
    ...
    // Replay historical daemon logs to the tmux session (last 200 lines only)
    // Note: We replay logs BEFORE enabling pipe-pane to avoid capturing
    // the tail output back into the log file (which would cause duplication)
    if let Ok(log_path) = gflow::core::get_daemon_log_file_path() {
        ...
        // Wait for tail command to complete before sending next command
        // tail -n 200 is fast, so 200ms should be enough
