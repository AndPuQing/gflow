...
/// Event types for job state transitions and actions
#[derive(Debug, Deserialize, Serialize, Clone, Copy, PartialEq, Eq, Display, EnumString)]
pub enum EventType {
    Created,         // Job initially submitted
    StateTransition, // State changed (e.g., Queued → Running)
    GPUAssignment,   // GPUs assigned to job
    GPURelease,      // GPUs released from job
    ParameterUpdate, // Job parameters changed
    Hold,            // Job put on hold
    Release,         // Job released from hold
}

/// Job event for audit trail and history tracking
#[derive(Debug, Deserialize, Serialize, Clone)]
pub struct JobEvent {
    pub id: Option<i64>,             // Auto-increment ID from database
    pub job_id: u32,                 // Job this event belongs to
    pub event_type: EventType,       // Type of event
    pub timestamp: SystemTime,       // When the event occurred
    pub old_state: Option<JobState>, // Previous state (for StateTransition)
    pub new_state: Option<JobState>, // New state (for StateTransition)
    pub reason: Option<String>,      // Optional reason/context
    pub gpu_ids: Option<Vec<u32>>,   // GPU IDs (for GPUAssignment/Release)
    pub parameters: Option<HashMap<String, String>>, // Parameters (for ParameterUpdate)
}
...
impl JobEvent {
    ...
    pub fn created(job_id: u32, initial_state: JobState) -> Self {
        Self {
            id: None,
            job_id,
    ...
    /// Create a new GPUAssignment event
    pub fn gpu_assignment(job_id: u32, gpu_ids: Vec<u32>) -> Self {
        Self {
            id: None,
            job_id,
            event_type: EventType::GPUAssignment,
            timestamp: SystemTime::now(),
            old_state: None,
            new_state: None,
            reason: None,
            gpu_ids: Some(gpu_ids),
            parameters: None,
        }
    }

    /// Create a new GPURelease event
    pub fn gpu_release(job_id: u32, gpu_ids: Vec<u32>) -> Self {
        Self {
            id: None,
            job_id,
            event_type: EventType::GPURelease,
            timestamp: SystemTime::now(),
            old_state: None,
            new_state: None,
            reason: None,
    ...
    pub fn hold(job_id: u32, reason: Option<String>) -> Self {
        Self {
            id: None,
            job_id,
    ...
    pub fn release(job_id: u32, reason: Option<String>) -> Self {
        Self {
            id: None,
            job_id,
