...
/// Benchmark HashMap insertion only (pre-created jobs)
fn bench_hashmap_insert_only(c: &mut Criterion) {
    let mut group = c.benchmark_group("bottleneck/hashmap_insert");
    group.sample_size(10);
    ...
    for size in [10_000, 25_000, 50_000, 75_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::new("jobs", size), &jobs, |b, jobs| {
            b.iter(|| {
                let mut map: HashMap<u32, Job> = HashMap::new();
...
/// Benchmark HashMap insertion with pre-allocated capacity
fn bench_hashmap_insert_preallocated(c: &mut Criterion) {
    let mut group = c.benchmark_group("bottleneck/hashmap_insert_prealloc");
    group.sample_size(10);
